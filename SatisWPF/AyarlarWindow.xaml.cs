...
public class CanliUrunModel
    ...
    public partial class AyarlarWindow : Window
    {
        ...
        private void chkPasifleriGoster_Changed(object sender, RoutedEventArgs e)
        {
            UrunleriListele();
            
            if (btnUrunIslem == null) return;
            if (chkPasifleriGoster.IsChecked == true)
            {
                btnUrunIslem.Content = "♻️  Seçili Ürünü Kurtar";
                btnUrunIslem.Background = (System.Windows.Media.Brush)FindResource("AccentGreen");
            }
            else
            {
                btnUrunIslem.Content = "🗑️  Seçili Ürünü Sil";
                btnUrunIslem.Background = (System.Windows.Media.Brush)FindResource("AccentRed");
            }
        }
        
        private void chkPasifMarkalariGoster_Changed(object sender, RoutedEventArgs e)
        {
            MarkalariYukle();
            
            if (btnMarkaIslem == null) return;
            if (chkPasifMarkalariGoster.IsChecked == true)
            {
                btnMarkaIslem.Content = "♻️ Kurtar";
                btnMarkaIslem.Background = (System.Windows.Media.Brush)FindResource("AccentGreen");
            }
            else
            {
                btnMarkaIslem.Content = "🗑️ Sil";
                btnMarkaIslem.Background = (System.Windows.Media.Brush)FindResource("AccentRed");
            }
        }

        private void UrunleriListele()
        {
            if (cmbKategoriFiltre == null || txtUrunAra == null) return;
            
            int turID = 0;
            if (cmbKategoriFiltre.SelectedValue != null && int.TryParse(cmbKategoriFiltre.SelectedValue.ToString(), out int parsedID))
            {
                turID = parsedID;
            }

            string arama = txtUrunAra.Text.Trim();
            ...
            try
            {
                ...
                {
                    q += "AND (u.UrunAdi LIKE @arama OR m.MarkaAdi LIKE @arama OR u.Barkod LIKE @arama) ";
                }

                q += "ORDER BY m.MarkaAdi, u.UrunAdi";

                var cmd75 = new SqlCommand(q, conn);
                if (turID > 0) cmd75.Parameters.AddWithValue("@turID", turID);
                if (!string.IsNullOrWhiteSpace(arama)) cmd75.Parameters.AddWithValue("@arama", "%" + arama + "%");
                cmd75.Parameters.AddWithValue("@aktifDurum", chkPasifleriGoster?.IsChecked == true ? 0 : 1);
            ...
            var liste = tablo.AsEnumerable().Select(r => new
            {
                ...
                TamAdi = (chkPasifleriGoster?.IsChecked == true ? "[PASİF] " : "") + $"{r["MarkaAdi"]} {r["UrunAdi"]}",
        ...
        private void MarkalariYukle()
        {
            ...
            {
                using var conn = new SqlConnection(_connStr);
                int aktifDurum = chkPasifMarkalariGoster?.IsChecked == true ? 0 : 1;
                using var cmd = new SqlCommand("SELECT MarkaID, MarkaAdi FROM Markalar WHERE AktifMi = @aktifDurum ORDER BY MarkaAdi", conn);
                cmd.Parameters.AddWithValue("@aktifDurum", aktifDurum);
        ...
        {
            if (_seciliUrunID < 0) { MessageBox.Show("Önce listeden bir ürün seçin.", "Uyarı"); return; }
            ...
            try
            {
                ...
                bool pasifMod = chkPasifleriGoster?.IsChecked == true;
                string baslik = pasifMod ? "Ürün Kurtar" : "Ürün Sil";
                string mesaj = pasifMod 
                    ? "Seçili ürünü tekrar satışa açmak (aktif etmek) istediğinize emin misiniz?" 
                    : "Seçili ürünü satıştan kaldırmak (pasife almak) istediğinize emin misiniz?\n(Geçmiş raporlarınız etkilenmeyecektir.)";

                var onay = MessageBox.Show(mesaj, baslik, MessageBoxButton.YesNo, MessageBoxImage.Question);
                if (onay != MessageBoxResult.Yes) return;

                int yeniDurum = pasifMod ? 1 : 0;
                using var cmdDel = new SqlCommand("UPDATE Urunler SET AktifMi = @durum WHERE UrunID = @id", conn);
                cmdDel.Parameters.AddWithValue("@id", _seciliUrunID);
                cmdDel.Parameters.AddWithValue("@durum", yeniDurum);
                cmdDel.ExecuteNonQuery();

                if (pasifMod)
                {
                    // Ürün kurtarılıyorsa, bağlı olduğu markayı da otomatik olarak aktif et (kurtar)
                    using var cmdMarka = new SqlCommand("UPDATE Markalar SET AktifMi = 1 WHERE MarkaID = (SELECT MarkaID FROM Urunler WHERE UrunID = @id)", conn);
                    cmdMarka.Parameters.AddWithValue("@id", _seciliUrunID);
                    cmdMarka.ExecuteNonQuery();
                }

                MessageBox.Show(pasifMod ? "✅ Ürün başarıyla aktif edildi ve bağlı olduğu marka da otomatik olarak kurtarıldı!" : "✅ Ürün başarıyla pasife alındı!", "Sistem");
                _seciliUrunID = -1;
                UrunleriListele();
                MarkalariYukle(); // Marka listesini de yenilemek iyi olur
            }
        ...
        {
            if (cmbMarkaSil.SelectedValue == null) { MessageBox.Show("Lütfen bir marka seçin.", "Uyarı"); return; }
            ...
            try
            {
                ...
                bool pasifMod = chkPasifMarkalariGoster?.IsChecked == true;
                string baslik = pasifMod ? "Marka Kurtar" : "Marka Sil";
                string mesaj = pasifMod
                    ? $"'{markaAdi}' markasını kurtarmak (aktif etmek) istediğinize emin misiniz?"
                    : $"'{markaAdi}' markasını ve ona bağlı tüm ürünleri satıştan kaldırmak (pasife almak) istediğinize emin misiniz?\n(Geçmiş raporlarınız etkilenmeyecektir.)";

                var onay = MessageBox.Show(mesaj, baslik, MessageBoxButton.YesNo, MessageBoxImage.Warning);
                if (onay != MessageBoxResult.Yes) return;

                int yeniDurum = pasifMod ? 1 : 0;
                new SqlCommand($"UPDATE Markalar SET AktifMi = {yeniDurum} WHERE MarkaID = {markaID}", conn).ExecuteNonQuery();
                
                if (!pasifMod)
                {
                    // Siliniyorsa içindeki ürünleri de sil
                    new SqlCommand($"UPDATE Urunler SET AktifMi = 0 WHERE MarkaID = {markaID}", conn).ExecuteNonQuery();
                    MessageBox.Show($"✅ '{markaAdi}' markasına ait tüm ürünler pasife alındı!", "Sistem");
                }
                else
                {
                    // Kurtarılıyorsa soralım
                    var urunleriDeKurtar = MessageBox.Show("Bu markaya ait daha önceden pasife alınmış ürünler de otomatik olarak aktif edilsin mi?", "Ürünleri Kurtar", MessageBoxButton.YesNo, MessageBoxImage.Question);
                    if (urunleriDeKurtar == MessageBoxResult.Yes)
                    {
                        new SqlCommand($"UPDATE Urunler SET AktifMi = 1 WHERE MarkaID = {markaID}", conn).ExecuteNonQuery();
                    }
                    MessageBox.Show($"✅ '{markaAdi}' markası başarıyla kurtarıldı!", "Sistem");
                }
        ...
        private void btnYeniMarkaEkle_Click(object sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                conn.Open();
                using var cmd = new SqlCommand(
                    "IF NOT EXISTS (SELECT 1 FROM Markalar WHERE MarkaAdi=@ad AND AktifMi=1) " +
        ...
        private void KategorileriDBdenYukle()
        {
            try
            {
                ...
                var dtFiltre = dt.Copy();
                DataRow dr = dtFiltre.NewRow();
                dr["TurID"] = 0;
                dr["TurAdi"] = "Tümü";
                dtFiltre.Rows.InsertAt(dr, 0);

                // Tüm ComboBox'ları dinamik olarak güncelle
                cmbKategoriFiltre.ItemsSource = dtFiltre.DefaultView;
        ...
        private void VeritabanindanCanliVerileriCek()
        {
            ...
            try
            {
                ...
                string kritikStokQuery = @"SELECT u.UrunID, m.MarkaAdi + ' ' + u.UrunAdi AS UrunAdi, u.MevcutStok
                                           ...
                                           WHERE u.MevcutStok <= 3 AND u.AktifMi = 1 ORDER BY u.MevcutStok ASC";
